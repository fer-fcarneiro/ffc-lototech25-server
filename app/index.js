...
export default function Index() {
  ...
  */
// VERSÃO DO INDEX.JS PARA ELIMINAR PROBLEMA DE INCIALIZAÇÃO E
//  CONSULTA SERVIDOR PARA INICIALIZAÇÃO,
import { useRouter } from "expo-router";
...
export default function Index() {
  ...
  async function iniciarApp() {
    try {
      ...
      if (planoLocal === "pro" || planoLocal === "free") {
        setPlano(planoLocal);
      }
    } catch (error) {}
    ...
    atualizarPlanoServidor();
  }

  async function atualizarPlanoServidor() {
    try {
      const planoServidor = await verificarPlanoServidor();

      if (planoServidor === "pro" || planoServidor === "free") {
        setPlano(planoServidor);
        await AsyncStorage.setItem("planoUsuario", planoServidor);
      }
    } catch (error) {
      setTimeout(() => {
        tentarNovamente();
